...
class JobExecutor:
    def __init__(self, job_id, job_type, job_template, run_params, job_log):
        ...
        elif job_type in ('flink', 'flink-batch', 'flink-stream'):
    ...
    def __exe_sync_datax(self):
        ...
        for j in self.job_list:
            ...
            for p in sync_param:
                ...
                writer = repo.get_datax_writer(server_id=j['to_server_id'], before_write=before_write,after_write=after_write,db_name=j['to_db_name'], table_name=j['to_table'], columns=columns)
    ...
    def __exe(self):
        if self.job_type == 'py':
            ...
            logs = self.__exe_sql()
        elif self.job_type in ('flink', 'flink-batch', 'flink-stream'):
